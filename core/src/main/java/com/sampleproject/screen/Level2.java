...
public class Level2 implements Screen, InputProcessor, Serializable {
    ...
    public Level2(Main main, UserManager.User user) {
    ...
    public Level2(Main main,UserManager.User user,ArrayList<Block> allOldBlock, ArrayList<Rock> allOldRock, ArrayList<Pig> allOldPig, Queue<BirdInterface> allOldBirds) {
        world = new World(new Vector2(0, -9.81f), true);
        world.setContactListener(new BirdBlockContactListener());
        camera = new OrthographicCamera(WORLD_WIDTH_METERS,WORLD_HEIGHT_METERS);
        camera.update();
        ...
        for (Block b : allOldBlock) {
            ...
            nb.setHealth(b.getHealth() + 100);
            nb.addBlock(b.getInitialPosition().x * 32,b.getInitialPosition().y * 32,(float) b.width , (float) b.height);
        ...
        for (Rock r : allOldRock) {
            ...
            nr.addRock(r.getInitialPosition().x * 32,r.getInitialPosition().y * 32,r.width,r.height,r.getInitialPosition().z * 32);
            nr.addDamage();
        }
    ...
    public void show() {
        ...
        highScoreLabel = new Label("High Score: " + user.getLevel2score(), labelStyle);
    ...
    void init() {
        ...
        Pig Pig1 = new Pig(stage,world,"normal");
        Pig1.addPig(1370,270,allPig);
        Pig1.addDamage();
        Pig Pig3 = new Pig(stage,world,"helmet");
        Pig3.addPig(1470,132,allPig);
        Pig3.addDamage();
        Pig Pig4 = new Pig(stage,world,"helmet");
        Pig4.addPig(1270,132,allPig);
        Pig4.addDamage();

        Pig Pig2 = new Pig(stage,world,"king");
        Pig2.addPig(1350,225+28+152+25,allPig);
        Pig2.addDamage();

        RedBird redBird1 = new RedBird(stage,world,viewport,user);
    ...
    public void render(float v) {
        ...
        if (allPig.isEmpty()) {
            if (!isWaiting) {
                ...
                if (waitTime >= WAIT_DURATION) {
                    ...
                    backgroundMusic.stop();
                    main.setScreen(new Won(main, 2, score,user));
                    return;
                }
            }
        }
        ...
        if (allBirds.isEmpty() && !allPig.isEmpty()) {
            if (!isWaiting) {
                ...
                if (waitTime >= WAIT_DURATION) {
                    backgroundMusic.stop();
                    main.setScreen(new Loss(main, 2,user));
                    return;
                }
            }
        }
...
//        }
        ...
        if(Gdx.input.isKeyPressed(Input.Keys.W)) {
            main.setScreen(new Won(main,2,0,user));
        }
    ...
    public static Level2 loadGame(String filename) {
        System.out.println("Loading level1");
        Level2 level = null;
        try (FileInputStream fileIn = new FileInputStream(filename);
             ObjectInputStream in = new ObjectInputStream(fileIn)) {
            level = (Level2) in.readObject(); // Deserialize the object
